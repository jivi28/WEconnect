...
--
-- This network branch's schema.sql (section 16) already extended
-- handle_new_user() to also insert `username`, and made that column NOT NULL
-- (section 18) — a constraint the qrcode branch's schema never had. Carrying
-- over the qrcode branch's version of this function as-is would drop
-- `username` from the insert and break every signup, QR or not. Preserve it.
